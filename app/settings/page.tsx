...
export default function Settings() {
  ...
  useEffect(() => {
    // Redirect non-admin users even if they try to access directly
    if (!loading && !isAdmin) {
      router.push('/')
    }
  ...
  // Additional check to prevent content flash
  if (!isAdmin) {
    return (
      <div className="min-h-screen flex items-center justify-center flex-col gap-4">
        <div className="text-xl font-medium text-red-500">Acesso restrito</div>
        <Link 
          href="/"
